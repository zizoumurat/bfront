...
export class PerformanceManagementComponent implements OnInit {
    ...
    supplierActionStatusOptions: ListItemModel[] = [];
    ...
    constructor(
        ...
        this.supplierActionStatusOptions = Object.keys(SupplierActionStatusEnum)
            .filter((key) => isNaN(Number(key))) // Sayısal değerleri filtrele
            .map((key) => ({
                id: SupplierActionStatusEnum[key as keyof typeof SupplierActionStatusEnum],
                name: this.translateService.instant(`SupplierActionStatus.${key}`)
            }));
    }
    ...
    async getActionList(supplierId: number) {
        this.supplierActions = await this.actionService.getListBySupplier(supplierId);
    }
    ...
    createActionForm() {
        ...
        this.actionForm = this.fb.group({
            ...
            supplierActionStatus: control(null, [Validators.required]),
            supplierNotes: control(null, [Validators.required]),
        });
    }
    ...
    handleCreateAction(action: SupplierActionModel) {
        this.actionForm.patchValue({ id: this.selectedRow?.id, ...action });
    ...
    onSubmit() {
        ...
        this.actionService.update(data)
            .then(() => {
                ...
                this.getActionList(this.selectedRow.id);
