...
export class PaymentListComponent implements OnInit {
    ...
    constructor(
        ...
        @Inject(PAYMETNLIST_SERVICE) protected paymentListService: IPaymentListService,
    ...
    createActionForm() {
        ...
        this.createForm = this.fb.group({
            ...
            orderIdList: control(null, [Validators.required]),
        });
    }

    ngOnInit(): void {
        this.createActionForm();
        this.searchObject = { status: OrderStatusEnum.Delivered, isPaymentList: true };

        this.filterButtons = ['allSupplierInvoices', 'overdueSupplierInvoices', 'upcomingSupplierInvoices'];
    }

    submitFilter() {
        this.searchObject = { isPaymentList: true, ...this.filterForm.value };
    }
    ...
    applyFilter(index: number) {
        ...
        if (index === 0) {
            this.searchObject = { ...this.searchObject, invoiceDateOrder: null };
        }

        if (index === 1) {
            this.searchObject = { ...this.searchObject, invoiceDateOrder: false };
        }

        if (index === 2) {
            this.searchObject = { ...this.searchObject, invoiceDateOrder: true };
        }
    ...
    createPaymentList() {
        this.createForm.get('orderIdList')?.setValue(this.selectedOrders.map(x => x.id));  
    ...
    async onSubmit() {
        ...
        await this.paymentListService.create(data)
