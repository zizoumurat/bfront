...
import { ApprovalStatus, ContractStatusEnum, RequestStateEnum } from '../enums/request.enum';
...
export class FormatColumnPipe implements PipeTransform {
    ...
    transform(value: any, formatType?: FormatEnum, parameter?: any): string | SafeHtml {
        switch (formatType) {
            ...
            case FormatEnum.approvals:
                return this.getApprovalUsers(value);
            default:
                return value;
        }
    }
    ...
    private getApprovalUsers(approvalUsers: { id: number; userName: string; status: number }[]): string {
        const statusIcons: Record<number, { icon: string, tooltip: string, color: string }> = {
            [ApprovalStatus.Pending]: { icon: "pi pi-clock", tooltip: "Beklemede", color: "text-yellow-500" },
            [ApprovalStatus.Approved]: { icon: "pi pi-check", tooltip: "Onaylandı", color: "text-green-500" },
            [ApprovalStatus.Rejected]: { icon: "pi pi-times", tooltip: "Reddedildi", color: "text-red-500" },
        };

        return approvalUsers.map(user => {
            const { icon, tooltip, color } = statusIcons[user.status] || { icon: "pi pi-question", tooltip: "Bilinmiyor", color: "text-gray-500" };

            return `
                <div class="approval-container flex items-center gap-2">
                    <span>${user.userName}</span>
                    <i class="${icon} ${color}" title="${tooltip}"></i>
                </div>
            `;
        }).join('');
    }
